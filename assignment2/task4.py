...
from task2a import cross_entropy_loss, SoftmaxModel, one_hot_encode, pre_process_images, get_training_statistics
...
def calculate_accuracy(X: np.ndarray, targets: np.ndarray, model: SoftmaxModel) -> float:
    ...
    # Returns the mean of an array of true (1) or false (0) for the predictions
    return (predict == targets).mean()
    ...
    def train_step(self, X_batch: np.ndarray, Y_batch: np.ndarray):
        ...
        if self.use_momentum:
            for ind, w in enumerate(self.model.ws):
                delta_ws = self.model.grads[ind] + \
                    self.momentum_gamma * self.model.delta_ws[ind]
                self.model.ws[ind] = w - \
                    self.learning_rate*delta_ws
                self.model.delta_ws[ind] = delta_ws
        else:
            for ind, w in enumerate(self.model.ws):
                self.model.ws[ind] = w - self.self.learning_rate*(self.model.grads[ind])
